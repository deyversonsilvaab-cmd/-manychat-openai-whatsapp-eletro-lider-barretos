...
const GENERIC_ITEM_PATTERNS = [
  { regex: /fechadura(?:s)?(?:\s+el[ée]trica(?:s)?)?/i, name: (m) => (norm(m[0]).includes("ELETRICA") ? "FECHADURA ELETRICA" : "FECHADURA") },
  { regex: /tomada(?:s)?/i, name: () => "TOMADA" },
  { regex: /interruptor(?:es)?/i, name: () => "INTERRUPTOR" },
  { regex: /l[âa]mpada(?:s)?/i, name: () => "LAMPADA" },
  { regex: /chuveiro(?:s)?/i, name: () => "CHUVEIRO" },
  { regex: /ventilador(?:es)?/i, name: () => "VENTILADOR" },
  { regex: /reator(?:es)?/i, name: () => "REATOR" },
  { regex: /eletroduto(?:s)?/i, name: () => "ELETRODUTO" },
  { regex: /disjuntor\s*dr/i, name: () => "DISJUNTOR DR" },
{ regex: /quadro\s+de\s+distribui[çc][ãa]o/i, name: () => "QUADRO DE DISTRIBUICAO" },
  { regex: /extens[ãa]o(?:\s+el[ée]trica)?/i, name: () => "EXTENSAO ELETRICA" },
  { regex: /fita\s+isolante/i, name: () => "FITA ISOLANTE" },
  { regex: /conector(?:es)?/i, name: () => "CONECTOR" }
  ];

function extractGenericItems(text) {
    const items = [];

  for (const { regex, name } of GENERIC_ITEM_PATTERNS) {
        const match = text.match(regex);
        if (!match) continue;

      const before = text.slice(Math.max(0, match.index - 25), match.index);
        const qtyMatch = before.match(/(\d+)\s*(?:un|und|unid|unidades?|pe[çc]as?|pcs?)?\s*(?:de\s*)?$/i);
        const qtd = qtyMatch ? `${qtyMatch[1]} PC` : "";

      addUnique(items, {
              descricao: name(match),
              quantidade: qtd,
              unidadeSolicitada: qtd ? "PC" : ""
      });
  }
...
export function extractItems(message = "") {
  ...
  for (const item of extractCableItems(text)) addUnique(items, item);
    ...
    for (const item of extractGenericItems(text)) addUnique(items, item);
...
export function customerWantsReset(message = "") {
    const text = norm(message);
    return [
          "CANCELAR PEDIDO", "CANCELA O PEDIDO", "CANCELAR A LISTA", "RECOMECAR",
          "COMECAR DE NOVO", "OUTRO PEDIDO", "ESQUECE ISSO", "ESQUECE A LISTA",
          "APAGA TUDO", "APAGAR TUDO", "LIMPAR PEDIDO", "LIMPAR A LISTA", "ZERAR PEDIDO"
        ].some((term) => text.includes(norm(term)));
}
